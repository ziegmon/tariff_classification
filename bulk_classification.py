...
def show_bulk_classification_page():
    ...
    if st.session_state.bulk_results_df is not None:
        ...
        successful = len(df[df['hs_code_1'].notna() & (df['hs_code_1'] != 'N/A') & (df['hs_code_1'] != 'ERROR') & (df['hs_code_1'] != '')])
        ...
        with col4:
            st.metric("Success Rate", f"{(successful/total_products)*100:.1f}%" if total_products > 0 else "0.0%")
        ...
        with col1:
            filter_country = st.selectbox("Filter by Country",
                                        ["All"] + sorted(list(df['input_country'].unique())))
        with col2:
        ...
        if filter_status == "Success":
            filtered_df = filtered_df[filtered_df['hs_code_1'].notna() &
                                    ...
                                    (filtered_df['hs_code_1'] != '') # Ensure it's not empty string
                                    ]
            ...
            with st.expander(f"👟 Product {original_index + 1}: {row['input_gender']}'s {row['input_product']} - {row['input_country']}"):
                ...
                with col2:
                    if st.button(f"🔄 Regenerate", key=f"regen_{original_index}"):
                        ...
                        # Trigger regeneration (already in session state due to the button click)
                ...
                if row['hs_code_1'] not in ['N/A', 'ERROR', '']: # Also check for empty string
                    ...
                    for i in range(1, 4):
                        hs_code = row[f'hs_code_{i}']
                        certainty = row[f'certainty_{i}']
                        reasoning = row[f'reasoning_{i}']
                        # New breakdown scores
                        hist_certainty = row[f'hist_certainty_{i}']
                        reasoning_score = row[f'reasoning_score_{i}']
                        format_score = row[f'format_score_{i}']
                        ...
                        if pd.notna(hs_code) and hs_code not in ['N/A', 'ERROR', '']:
                            ...
                            st.markdown(f"**Option {i}: N/A** (No valid HS code provided for this option)")
                    ...
                    if st.button("💾 Save Selection", key=f"save_{original_index}"):
                        if selection == "None of these":
                            ...
                            # Extract HS code from selection and find corresponding certainty
                            # Use the simplified selection text for matching
                            selected_hs_code_text = selection.split(": ")[1]
                            selected_option_num = -1
                            for i in range(1, 4):
                                if row[f'hs_code_{i}'] == selected_hs_code_text:
                                    selected_option_num = i
                                    break

                            if selected_option_num != -1:
                                certainty_value = row[f'certainty_{selected_option_num}']
                                st.session_state.product_selections[original_index] = {
                                    'status': 'selected',
                                    'hs_code': selected_hs_code_text,
                                    'option': selected_option_num,
                                    'certainty': certainty_value # Store the calculated certainty of the selected option
                                }
                            else:
                                st.warning(f"Could not find matching option for {selected_hs_code_text}. Please re-select.")
                                continue # Don't save if we can't map it back

                        st.success(f"Saved selection for Product {original_index + 1}")
                        st.rerun() # Rerun to reflect saved status if needed
                else:
            ...
            for original_index, selection in st.session_state.product_selections.items():
                ...
                if selection['status'] == 'selected':
                    ...
                    if product_row['hs_code_1'] == correct_hs_code:
                        top1_correct += 1
                    # Check if the selected code is any of the top 3
                    if correct_hs_code in [str(product_row['hs_code_1']), str(product_row['hs_code_2']), str(product_row['hs_code_3'])]:
                        top3_correct += 1
                    weighted_sum += selection['certainty'] # Add the certainty of the selected option
                else: # 'none' status, or manual entry not validated against options
                    # If a manual code was entered, we don't count its certainty towards the AI's weighted score.
                    # The 'certainty' in selection is 0 for 'none' status.
                    weighted_sum += 0
    ...
    if st.button("Generate Diagnostic Plot"):
        if not st.session_state.product_selections:
            ...
            for idx in selected_indices:
                ...
                row_idx_in_df = selected_df[selected_df['original_index'] == idx].index[0]

                if selection['status'] == 'selected':
                    selected_df.loc[row_idx_in_df, 'selected_status'] = 'selected'
                    selected_df.loc[row_idx_in_df, 'selected_option'] = selection['option']
                    selected_df.loc[row_idx_in_df, 'is_correct_top1'] = (selection['option'] == 1)
                    selected_df.loc[row_idx_in_df, 'is_correct_top3'] = True # If selected, it's one of the options
                    selected_df.loc[row_idx_in_df, 'selected_certainty'] = selection['certainty']
                else: # 'none' status
                    selected_df.loc[row_idx_in_df, 'selected_status'] = 'none'
                    selected_df.loc[row_idx_in_df, 'selected_option'] = None
                    selected_df.loc[row_idx_in_df, 'is_correct_top1'] = False
                    selected_df.loc[row_idx_in_df, 'is_correct_top3'] = False
                    selected_df.loc[row_idx_in_df, 'selected_certainty'] = 0
                    if 'correct_hs_code' in selection and selection['correct_hs_code']:
                        selected_df.loc[row_idx_in_df, 'manual_hs_code'] = selection['correct_hs_code']
    ...
    if st.session_state.accuracy_history:
        ...
        fig, ax = plt.subplots(figsize=(8, 4)) # Increased size for better readability
        ...
        ax.set_xlabel('Calculation Run', fontsize=10) # Added label
        ...
        buf.seek(0)
        st.image(buf.getvalue(), width=700) # Increased width in display
        ...
        if 'weighted_score' in history_df.columns:
            ...
            if not history_df_weighted.empty:
                ...
                ax2.set_xlabel('Calculation Run', fontsize=10) # Added label
    ...
    with st.expander("📖 How to Use Bulk Classification"):
        ...
        4.  **📊 Review Results** - Examine each product's classification options. Each option now shows a **total certainty score** along with a **breakdown** explaining its components:
            *   **Historical Similarity:** How well the proposed HS code aligns with classifications of similar products in your historical data.
            *   **Reasoning Detail Score:** Reflects the length and assumed comprehensiveness of the AI's provided reasoning (0-20 points).
        ...
        -   Each product gets 3 classification options with objective, externally calculated certainty scores.
